...
function runCurrency () {
  return new Promise(function (resolve) {
    let xhr = new XMLHttpRequest();
    xhr.open('GET', 'https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20yahoo.finance.xchange%20where%20pair%20in(%22USDBYN%22%2C%20%22USDEUR%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys');
    xhr.send();
    xhr.onreadystatechange = function () {
      if (xhr.status === 200 && xhr.readyState === 4) {
